...
const SecureChatApp = () => {
  ...
  const generateOrLoadKeyPair = async () => {
    ...
    const storedPrivateKey = JSON.parse(
    ...
    if (storedPrivateKey && storedPublicKey) {
      try {
        ...
        console.error(
          "Failed to import stored keys, generating new ones:",
          error
        );
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const decrypted = await Promise.all(
        data.map(async (msg) => {
          try {
            ...
            const keyHex = await exportKeyBytes(aesKey);
            console.log(
              `[${username}] Shared key with ${otherUsername}: ${keyHex}`
            );
      ...
      setMessages(
        decrypted.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
      );
    } catch (error) {
    ...
    return (
      <div style={{ padding: 20, fontFamily: "Arial", maxWidth: 400 }}>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          />
          <button onClick={registerUser} style={{ padding: 8, width: "100%" }}>
            Register
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ padding: 20, fontFamily: "Arial", maxWidth: 800 }}>
      ...
      >
        ...
        <button onClick={fetchUsers} style={{ padding: 8 }}>
          Refresh Users
        </button>
      </div>
                  ...
                  <li key={u.username} style={{ marginBottom: 5 }}>
                    <button
                      ...
                      style={{
                        ...
                        textAlign: "left",
                        backgroundColor:
                          selectedUser === u.username ? "#e3f2fd" : "white",
                      }}
                    >
                      {u.username}
                    </button>
                    ...
                    .map((m, i) => (
                      <div
                        ...
                        style={{
                          ...
                          padding: 8,
                          backgroundColor:
                            m.sender === username ? "#e8f5e8" : "white",
                          borderRadius: 4,
